...
fn test_effective_search_space_blastn_manual() {
    ...
    println!(
        "Effective query length: {:.2}",
        search_space.effective_query_len
    );
    println!(
        "Effective database length: {:.2}",
        search_space.effective_db_len
    );
    println!(
        "Effective search space: {:.2}",
...
fn test_effective_search_space_various_lengths() {
    ...
    for (q_len, db_len, db_num_seqs) in test_cases {
        let search_space =
            SearchSpace::for_database_search(q_len, db_len, db_num_seqs, &params, true);

        println!(
            "\nTest case: q_len={}, db_len={}, db_num_seqs={}",
