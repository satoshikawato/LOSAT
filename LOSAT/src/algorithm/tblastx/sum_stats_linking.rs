...
fn link_hsp_group_ncbi(
    ...
    length_adj_per_context: &[i64],
    eff_searchsp_per_context: &[i64],
    log_k_by_ctx: &[f64],  // NEW: Pre-computed log(K) per context
    pool_lh_helpers: &mut Vec<LhHelper>,  // NEW: Thread-local pool for lh_helper reuse
    pool_hsp_links: &mut Vec<HspLink>,     // NEW: Thread-local pool for hsp_link reuse
) -> Vec<UngappedHit> {
