#!/usr/bin/env python3
"""Save pinned NCBI mode-2 composition probabilities at score adjustment."""
...
PROBE = HERE / "ncbi_kappa_composition_trace.c"
CASES = ("seg_hard_query_20260924_default", "multi_query_20260924_default")
PREFIXES = (b"K_COMP_CALL\t", b"K_MATRIX\t", b"K_COMP\t", b"K_COMP_RESULT\t", b"K_ADJUSTED\t")


def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: run_ncbi_kappa_composition_trace.py NEW_OUTPUT_DIR")
    ...
    for case in CASES:
        ...
        with tempfile.TemporaryDirectory(prefix="tlosan-d-composition-") as tmp:
        ...
        assert any(line.startswith(b"K_COMP_RESULT\t") for line in selected), case
