#!/usr/bin/env python3
"""Save exact pinned NCBI ErfC calls for scaled TBLASTN Kappa diagnostics."""
...
PROBE = HERE / "ncbi_erfc_trace.c"


def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: run_ncbi_erfc_trace.py NEW_OUTPUT_DIR")
    ...
    for case in CASES:
        ...
        with tempfile.TemporaryDirectory(prefix="tlosan-d-erfc-") as tmp:
            traced = probe_run(command, PROBE, Path(tmp))
        lines = traced.stderr.splitlines(keepends=True)
        selected = [line for line in lines if line.startswith(b"D_ERFC\t")]
        assert traced.stdout == plain.stdout
        assert b"".join(line for line in lines if not line.startswith(b"D_ERFC\t")) == plain.stderr
        assert selected
