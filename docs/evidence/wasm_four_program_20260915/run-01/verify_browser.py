...
RUN = """async ({fixture, mode, threads, abort}) => {
  ...
  const options = { executionMode: mode, threadsPerJob: threads,
    totalThreadBudget: threads, concurrency: 1,
    sequences: {q:fixture.query,s:fixture.subject}, signal: controller.signal,
    onRuntimeStatus: status => statuses.push({state:status.state,mode:status.mode}) };
  const started=performance.now();
  const pending = service.runLosatPairsParallel([job], options);
  ...
  try { const results = await promise; return {text:results[0].text,statuses,elapsed_ms:performance.now()-started}; }
