...
impl ProteinDiagnosticCounters {
    ...
    pub fn print_summary(&self, min_ungapped_score: i32) {
        ...
        if mask_updates > 0 {
            ...
            eprintln!("  Mask updates:               {}", mask_updates);
        }
        ...
        if num_ext > 0 {
            ...
            eprintln!("    Maximum length:           {}", max_len);
            if max_len > 10000 {
        ...
        eprintln!("  Filtered (cutoff_score):     {}", cutoff_failed_count);
        if cutoff_failed_count > 0 {
            let min_score = self
                .ungapped_cutoff_failed_min_score
                .load(AtomicOrdering::Relaxed);
            let max_score = self
                .ungapped_cutoff_failed_max_score
                .load(AtomicOrdering::Relaxed);
            // Display score range if at least one value was updated
            if min_score != i32::MAX || max_score != i32::MIN {
                if min_score == i32::MAX {
                    eprintln!("    Score range:                N/A - {}", max_score);
                } else if max_score == i32::MIN {
                    eprintln!("    Score range:                {} - N/A", min_score);
                } else {
                    eprintln!(
                        "    Score range:                {} - {}",
                        min_score, max_score
                ...
                eprintln!("    Score range:                (not updated)");
            }
        }
        eprintln!(
            "  E-value passed:             {}",
