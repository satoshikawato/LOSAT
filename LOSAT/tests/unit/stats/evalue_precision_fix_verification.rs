...
fn test_weight_divisor_handling() {
    ...
    assert_eq!(
        result1,
        i32::MAX as f64,
        "weight_divisor == 0.0 should return i32::MAX"
    );
    ...
    assert!(
        result3 > 0.0 && result3.is_finite(),
        "Normal case should return finite positive value"
    );
    assert!(
        result3 <= i32::MAX as f64,
        "Result should not exceed i32::MAX"
    );
    ...
    assert!(
        result4 > result3,
        "Smaller weight_divisor should give larger result"
    );
