#!/usr/bin/env python3
"""Build fixed mixed-case subjects and pinned local NCBI output."""
...
def main() -> None:
    ...
    (out / 'subjects.fna').write_text(
        f'>upper\n{core}\n>lower\n{core.lower()}\n'
        f'>middle\n{core[:90]}{core[90:270].lower()}{core[270:]}\n')
    ...
    (out / 'lowercase_fields.out').write_bytes(result.stdout)
    (out / 'lowercase_fields.stderr').write_bytes(result.stderr)
    (out / 'oracle_manifest.txt').write_text(
        ...
        f'Command: {cmd!r}\n')
    files = ('query.faa', 'subjects.fna', 'lowercase_fields.out',
             'lowercase_fields.stderr', 'oracle_manifest.txt')
    (out / 'oracle_outputs.sha256').write_text(''.join(
        f'{hashlib.sha256((out / name).read_bytes()).hexdigest()}  {name}\n'
        for name in files))
    print('NCBI lowercase oracle rows:', len(result.stdout.splitlines()))
