# NCBI reference: c++/src/algo/blast/core/link_hsps.c:827-895,955-980
# H2_index = next_larger; H_hsp_link = H2; H->hsp->evalue = evalue;
# Compare actual separately compiled baseline/candidate linking state.
from pathlib import Path
import shutil,subprocess,json,hashlib
b=Path(__file__).resolve().parent
rel=Path('LOSAT/src/algorithm/tblastx/sum_stats_linking/linking.rs')
test=r'''
...
'''
(b/'x2_frozen_group_test.rs').write_text(test)
for version in ['baseline','X2']:
 dest=b/(version+'state-source');shutil.copytree(b/(version+'-source'),dest,dirs_exist_ok=True)
 p=dest/rel;s=p.read_text();s=s[:s.rfind('\n}')]+test+s[s.rfind('\n}'):]
 needle='let b0 = sum <= h_sum;';assert s.count(needle)==1
 s=s.replace(needle,needle+'\n                            println!("STATE VISIT {} {} {} {} {} {} {} {}", i, current_idx, next_larger, b0, h_sum, h_num, h_link, h_xsum.to_bits());')
 needle='let new_sum = h_sum + (i_score - cutoff_big);';assert s.count(needle)==1
 s=s.replace(needle,'println!("STATE CHOICE {} {} {} {} {}", i, h_sum, h_num, h_link, h_xsum.to_bits());\n                '+needle)
 p.write_text(s)
 cmd=['cargo','test','--offline','--locked','--lib','--target-dir',str(b/(version+'state-unit-target')),'x2_frozen_group_transcript','--','--nocapture','--test-threads=1']
 with (b/(version+'state.log')).open('w') as f:subprocess.run(cmd,cwd=dest/'LOSAT',stdout=f,stderr=subprocess.STDOUT,check=True)
 lines=[l[l.index('STATE '):] for l in (b/(version+'state.log')).read_text().splitlines() if 'STATE ' in l]
 (b/(version+'state.txt')).write_text('\n'.join(lines)+'\n')
a=(b/'baselinestate.txt').read_bytes();c=(b/'X2state.txt').read_bytes()
record={'equal':a==c,'lines':len(a.splitlines()),'sha256':hashlib.sha256(a).hexdigest(),'cases':128}
(b/'X2-state-comparison.json').write_text(json.dumps(record,indent=2)+'\n');print(record);assert a==c
