...
p=argparse.ArgumentParser();p.add_argument('version');p.add_argument('--threads',type=int,nargs='+',default=[1,2,4,8]);a=p.parse_args();out=b/(a.version+'-edge-remainder');out.mkdir(exist_ok=False);f=fixtures(out/'fixtures')
...
for task in ['blastn','megablast']:
 cases.extend([(task+'-nohit','blastn',q,s,['-task',task]),(task+'-minus','blastn',rev,f['nuc1'],['-task',task])])
cases += [('tblastx-nohit','tblastx',q,s,[]),('blastp-nohit','blastp',pa,pb,[])]
for ev in ['10','100','10000']:cases.append(('tblastx-lc-e'+ev,'tblastx',t/'fasta/LC738874.fasta',t/'fasta/LC738875.fasta',['-evalue',ev]))
cases=cases[next(i for i,c in enumerate(cases) if c[0]=='blastn-minus'):]
